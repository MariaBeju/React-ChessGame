...
const arbiter = {
    ...
    getRegularMoves : function ({position,piece,row,col}) {
        if (piece.endsWith('n'))
            return getKnightMoves({position,row,col});
        if (piece.endsWith('b'))
            return getBishopMoves({position,piece,row,col});
        if (piece.endsWith('r'))
            return getRookMoves({position,piece,row,col});
        if (piece.endsWith('q'))
            return getQueenMoves({position,piece,row,col});
        if (piece.endsWith('k'))
            return getKingMoves({position,piece,row,col});
        if (piece.endsWith('p'))
            return getPawnMoves({position,piece,row,col})
    },

    getValidMoves : function ({position,castleDirection,prevPosition,piece,row,col}) {
        let moves = this.getRegularMoves({position,piece,row,col})
        ...
        moves.forEach(([x,y]) => {
            const positionAfterMove = 
                this.performMove({position,piece,row,col,x,y})
    ...
    insufficientMaterial : function(position) {
        ...
        const pieces = 
            position.reduce((acc,row) => 
                acc = [
                    ...acc,
                    ...row.filter(spot => spot)
                ],[])
